...
import com.example.telegramdiplom.ui.screeens.groups.settings.ShowMembersFragment
...
import com.example.telegramdiplom.database.getCommonModel as getCommonModel


class GroupChatFragment(private val groups: CommonModel) :
    BaseFragment(R.layout.fragment_single_chat) {
    private lateinit var mListenerInfoToolbar: AppValueEventListener
    private lateinit var mReceivingUser: UserModel
    private lateinit var mToolbarInfo:View
    private lateinit var mRefUser: DatabaseReference
    private val mRefMembers = REF_DATABASE_ROOT.child(NODE_GROUPS).child(groups.id).child(NODE_MEMBERS)
    ...
    private val mRefUsersCount = REF_DATABASE_ROOT.child(NODE_USERS)
    private var mListItems = mutableListOf<String>()
                ...
                if (checkPermission(RECORD_AUDIO)) {
                    if(event.action == MotionEvent.ACTION_DOWN){
                        ...
                        mAppVoiceRecorder.stopRecord(){file,messageKey ->
                            uploadFileToStorageGroup(Uri.fromFile(file),messageKey,groups.id, TYPE_MESSAGE_VOICE)
    ...
    private fun initInfoToolbar() {
        ...
        if(mReceivingUser.fullname.isEmpty()){
            mToolbarInfo.toolbar_chat_fullname.text = groups.fullname
        }else  mToolbarInfo.toolbar_chat_fullname.text = mReceivingUser.fullname
        mToolbarInfo.toolbar_chat_image.downloadAndSetImage(groups.photoUrl)
        mToolbarInfo.toolbar_chat_status.visibility=View.GONE
        mToolbarInfo.group_counts_button.visibility = View.VISIBLE
        mToolbarInfo.group_counts_button.setOnClickListener{
            getListContacts(1)
        }

    }

    private fun getListContacts(showMembersOrAddContact: Int){
        mRefMembers.addListenerForSingleValueEvent(AppValueEventListener { dataSnapshot ->   //получение List
            mListItems = dataSnapshot.children.map{it.key} as MutableList<String>
            for(item in mListItems) {
                mRefUsersCount.child(item).addListenerForSingleValueEvent(AppValueEventListener { dataSnapshot1 ->
                       listContacts.add(dataSnapshot1.getCommonModel())
                        if(listContacts.size == mListItems.size){
                            if(showMembersOrAddContact == 1) {
                                replaceFragment(
                                    ShowMembersFragment(
                                        listContacts,
                                        groups
                            ...
                            else replaceFragment(AddMembersFragment(groups, listContacts))
                        }
                    })
            }

        }) //получение map каждого пользователя в список из NODE_MAIN_LIST и преобразование их в CommonModel
        ...
        if(data!=null){
            when(requestCode){
                CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE -> {
                    ...
                    val messageKey = getMessageKey(groups.id)
                    uploadFileToStorageGroup(uri,messageKey,groups.id, TYPE_MESSAGE_IMAGE)
                    mSmoothScrollToPosition = true
                }
                PICK_FILE_REQUEST_CODE ->{
                    ...
                    uploadFileToStorageGroup(uri,messageKey,groups.id, TYPE_MESSAGE_FILE, filename)
                    mSmoothScrollToPosition = true
                }
        }
        }
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) { //Создание выпадающего меню
        activity?.menuInflater?.inflate(R.menu.group_chat_action_menu,menu) // ?-безопасный вызов
        mRefMembers.child(CURRENT_UID).addListenerForSingleValueEvent(AppValueEventListener{
            if(it.value?.equals("creator")!!){
                menu.findItem(R.id.menu_add_member_chat).setVisible(true)
            }
        })
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean { //Listener выбора пунктов выпадающего меню
        ...
        when(item.itemId){
        R.id.menu_delete_chat -> deleteChat(groups.id){
            showToast("Чат удален")
            replaceFragment(MainListFragment())
        }

            R.id.menu_add_member_chat ->{
               getListContacts(2)
            }
        }
        return  true
